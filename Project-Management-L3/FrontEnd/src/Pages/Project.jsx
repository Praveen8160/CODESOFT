  ...
  return (
    <div className="flex bg-slate-400 h-screen justify-center items-center">
      <div className="bg-slate-600 rounded-2xl min-h-[40rem] w-full sm:w-11/12 mx-5 md:w-4/5 lg:w-3/5 xl:w-2/3">
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-center my-2">{project.name}</h1>
        <hr />
        ...
        <div className="flex flex-col">
          <div className="flex m-4 items-center">
            ...
            {isModalOpen && <ModalTask onClose={handleCloseModal} pid={id} />}
            <h1 className="text-sm sm:text-lg font-semibold ml-3">Add Task</h1>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 lg:gap-14 mx-3">
            {project.tasks &&
              project.tasks.map((item) => (
                <div key={item._id} className="bg-slate-300 shadow-md p-4 border-gray-700 rounded-md">
                  ...
                  <p className="mb-2">
                    <strong>Assigned To:</strong> {item.assignedTo.username}
                  </p>
                  <p>
